...
import { Button, Card, Divider, Form, Input, Modal, Select, Table, message } from 'antd';

import Edit from './edit';
import { Columns } from './options';
import { FilterItemAttr } from '@/utils/constant';
import { fetchCategoryList, deleteCategory, updateCategoryStatus, fetchLanguageListAll } from '@/services';
...
const Index = () => {
  ...
  const fetchData = (pageOrPagination, nextSorter) => {
    ...
    setLoading(true);
    fetchCategoryList({ ...params }).then((res) => {
      if (res.code === 100000) {
        setPagination({
  ...
  const handleDelete = (record) => {
    Modal.confirm({
      ...
      onOk: () => {
        setLoading(true);
        deleteCategory({ id: record?.id }).then((res) => {
          if (res.code === 100000) {
            fetchData();
  ...
  return (
    <>
      ...
      <Card>
        <Edit title="新建" languageList={languageList}>
          ...
          columns={[
            ...
            ),
            {
              ...
              render: (_, record) => {
                return (
                  <>
                    <Edit title="编辑" record={record} languageList={languageList} onSuccess={() => fetchData()}>
                      <a>编辑</a>
                    </Edit>
                    <Divider type="vertical" />
                    <a onClick={() => handleDelete(record)} style={{ color: '#ff4d4f' }}>删除</a>
                    <Divider type="vertical" />
                    <Edit title="复制" record={record} copy={true} onSuccess={() => fetchData()} languageList={languageList}>
                      <a>复制</a>
                    </Edit>
                  </>
                );
